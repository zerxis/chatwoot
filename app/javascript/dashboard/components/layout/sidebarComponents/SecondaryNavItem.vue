<template>
  <li v-show="isMenuItemVisible" class="mt-1">
    <div v-if="hasSubMenu" class="flex justify-between">
      <span
        class="text-sm text-slate-700 dark:text-slate-100 font-semibold my-2 px-2 pt-1"
      >
    ...
    <router-link
      v-else
      class="rounded-lg leading-4 font-medium flex items-center p-2 m-0 text-sm text-slate-700 hover:bg-slate-25 hover:text-bg-50"
      :class="computedClass"
