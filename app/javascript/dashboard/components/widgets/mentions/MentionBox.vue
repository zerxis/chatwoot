<template>
  ...
  >
    <ul class="vertical dropdown menu">
      ...
      >
        <button
          class="flex group flex-col gap-0.5 overflow-hidden cursor-pointer items-start py-4 px-3.5 justify-center w-full h-full text-left hover:bg-woot-25 dark:hover:bg-woot-800 border-b border-solid border-slate-100 dark:border-slate-700"
          :class="{
        ...
        >
          ...
          <p
            class="text-slate-700 dark:text-slate-300 group-hover:text-woot-500 dark:group-hover:text-woot-500 mb-0 text-xs overflow-hidden text-ellipsis whitespace-nowrap min-w-0 max-w-full"
            :class="{
              'text-woot-500 dark:text-woot-500': index === selectedIndex,
            }"
          >
